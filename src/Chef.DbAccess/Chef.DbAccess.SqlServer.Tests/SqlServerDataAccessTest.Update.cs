...
namespace Chef.DbAccess.SqlServer.Tests
{
    public partial class SqlServerDataAccessTest
    {
        ...
        [TestMethod]
        public async Task Test_UpdateAsync_Join_Four_Tables_use_QueryObject()
        {
            var suffix = new Random(Guid.NewGuid().GetHashCode()).Next(100, 1000).ToString();
            ...
            await clubDataAccess.InnerJoin(x => x.Self, (x, y) => x.Id == y.Id)
                .InnerJoin((x, y) => y.Self, (x, y, z) => y.Id == z.Id)
                .InnerJoin((x, y, z) => z.Self, (x, y, z, a) => z.Id == a.Id)
                .Where((x, y, z, a) => a.Id.Equals(15))
                .Set(() => new Club { Name = clubName })
                .UpdateAsync();
        ...
        [TestMethod]
        public async Task Test_UpdateAsync_Multiply_Join_Two_Tables_use_QueryObject()
        {
            var suffix = new Random(Guid.NewGuid().GetHashCode()).Next(100, 1000).ToString();
        ...
        [TestMethod]
        public async Task Test_UpdateAsync_Multiply_Join_Four_Tables_use_QueryObject()
        {
            var suffix = new Random(Guid.NewGuid().GetHashCode()).Next(100, 1000).ToString();
